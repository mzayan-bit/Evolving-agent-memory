...
def test_complete_inputs_and_no_mutation(tmp_path: Path) -> None:
...
def test_and_or_have_same_edges_different_sets(tmp_path: Path) -> None:
    a, b = dummy(), dummy()
...
def test_member_order_ignored(tmp_path: Path) -> None:
    a, b = dummy(), dummy()
...
def test_unknown_is_not_imputed_as_empty_support(tmp_path: Path) -> None:
    a, b = dummy(), dummy()
    b["support_sets_json"] = json.dumps(
...
def test_blank_rejected_and_partial_explicit(tmp_path: Path) -> None:
    a = dummy()
...
def test_evidence_mismatch_rejected(tmp_path: Path) -> None:
    a, b = dummy(), dummy()
    b["evidence_json"] = "[]"
...
def test_duplicate_state_rejected(tmp_path: Path) -> None:
    a = dummy()
...
def test_explicit_empty_vs_missing_distinguished(tmp_path: Path) -> None:
    a, b = dummy(), dummy()
    b["relations_json"] = "[]"
    result = agreement.analyze(*save_pair(tmp_path, a, b))
...
def test_historical_status_and_revision_sets(tmp_path: Path) -> None:
    a, b = dummy(), dummy()
    for row in [a, b]:
        row["checkpoint"] = "t1"
...
def test_empty_sets_not_inflated_to_perfect_f1() -> None:
